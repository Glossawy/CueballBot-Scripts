...
#
# Commands:
#   hubot poll people about <topic> with options <choiceOne, choiceTwo, ..., choiceN>
...
module.exports = (robot) ->
    ...
    removeArchivedPoll = (archiveName) ->
        robot.brain.archives = filterMap(robot.brain.archives, (name, v) -> name isnt archiveName)
        _saveBrain()
    ...
    isSuperAdmin = (user) -> return user in process.env.HUBOT_POLLING_SUPERS.split(':')
    ...
    class Poll
        ...
        getChoiceIndexOf: (user) -> return if user of @voters then @voters[user] else -1
        getChoiceOf: (user) -> 
    ...
    robot.respond /poll people about (".*?") with options ((.*?)+)/i, (msg) ->
        ...
        msg.reply "Your poll has been created! Details as follows:\n" +
                 ...
                 "Example: cueball vote in #{newPoll.getId()} for #{Math.floor(Math.random() * (choices.length-1)) + 1}"
    ...
    robot.respond /vote in (\w+) for ((.*?)+)/i, (msg) ->
        ...
        if poll?
            ...
            try
                ...
                msg.reply "You have successfully voted for '#{poll.getChoices()[decision-1]}' in the poll, '#{poll.getTopic()}'!"
            catch error
    ...
    robot.respond /show options for (\w+)/i, (msg) ->
        ...
        if poll?
            optStr = "[ #{poll.getChoices().map((val, i) -> "#{i+1}: #{val}").join(', ')} ]"
    ...
    robot.respond /show voters for (\w+)/i, (msg) ->
        ...
        if poll?
            ...
            resp = if voteTotal == 1 then "1 person has voted:" else "\n#{voteTotal} people have voted:" 
    ...
    robot.respond /what did I vote for in (\w+)/i, (msg) ->
        ...
        if poll?
            ...
            if choice?
                msg.reply "You voted for option #{choiceIdx}, '#{choice}', in the poll titled '#{poll.getTopic()}'"
                if poll.isVoteChangeable() then msg.send 'This vote can be changed.' else msg.send 'This vote can\'t be changed!'
            else
                msg.reply "You have not yet voted in '#{poll.getTopic()}'! Do so using it's ID! Which is '#{poll.getId()}'"
        else
            msg.reply "The poll with id '#{id}' is not appearing in my database..."

    robot.respond /show active polls/i, (msg) ->
        len = lengthMap(robot.brain.uuid_map)
        resp = "There #{if len == 1 then "is 1 poll" else "are #{len} polls"} active:\n"

        for own id, poll of robot.brain.uuid_map
            resp += "#{id}: #{if poll.getVotes() == 1 then "1 vote" else poll.getVotes() + " votes"}\t'#{poll.getTopic()}'\n"
    ...
    robot.respond /(end poll (\w+))( and archive as ((".*?")|('.*?')))?/i, (msg) ->
        ...
        if !archiveName? and msg.match[0].trim().length != 14
            msg.reply "If you are trying to archive please make sure to say 'and archive as \"<name here>\"'!\n"
            + "Since your request is not proper, the poll will not be deleted as a precaution."
            return
        ...
        if poll?
            if isArchived(archiveName)
                ...
                if isSuperAdmin(user) or poll.getCreator() is user
                    ...
                    msg.reply "The poll, '#{poll.getTopic()}' has been successfully removed."
                    if archiveName?
                        msg.send "Archived as '#{archiveName}'"
                else
                    msg.reply "#{user} you are not the poll creator (#{poll.getCreator()}) nor are you an admin!"
        else
            msg.reply "The poll with id '#{id}' is not appearing in my database..."
    ...
    robot.respond /delete ((".*?")|('.*?')) from archive/i, (msg) ->
        ...
        if poll?
            if isSuperAdmin(user) or poll.getCreator() is user
                removeArchivedPoll(archiveName)
                msg.reply "The poll, '#{poll.getTopic()}', has been permanently deleted!"
            else
                msg.reply "#{user} you are not the poll creator (#{poll.getCreator()}) nor are you an admin!"
        else
            msg.reply "The archived poll with name '#{archiveName}' is not appearing in my database..."

    robot.respond /view ((("|').*?("|'))( from archive)|(\w+))/i, (msg) ->
        fromArchive = msg.match[5].trim() is 'from archive'
        identifier = if fromArchive then msg.match[2].trim() else msg.match[2].trim().toUpperCase()
        poll = null
        ...
        if fromArchive
            ...
            if !poll?
                msg.reply "The archived poll with name '#{identifier}' is not appearing in my database..."
                return
        else
            poll = retrievePoll(identifier)
            if !poll?
                msg.reply "The poll with id '#{identifier}' is not appearing in my database..."
                return

        resp = 'Review is as follows,'
        poll.getChoices().map((v, i) -> return [i+1, v]).sort((a,b) -> return poll.getVotesForName(b[1]) - poll.getVotesForName(a[1]))
